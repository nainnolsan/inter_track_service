...
export const getSwipeHistory = async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const result = await query('SELECT job_id, status FROM job_interactions WHERE user_id = $1', [userId]);
...
export const swipeJob = async (req: AuthenticatedRequest, res: Response) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'No autenticado' });
        ...
        if (status === 'liked' && companyName && roleTitle) {
            ...
            let companyId;
            const companyResult = await query('SELECT id FROM companies WHERE user_id = $1 AND name = $2', [userId, companyName]);
            ...
            `, [userId, companyId, roleTitle, 'saved', url, location, 'Añadido desde Discovery (SaaS)']);
        }
